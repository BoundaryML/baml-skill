...
- Instance methods on `string`, `Array`, and `Map` are **snake_case**: `.to_lower_case()`, `.to_upper_case()`, `.replace_all()`, `.replace()`, `.trim()`, `.includes()`, `.starts_with()`, `.ends_with()`, `.index_of()`, `.char_at()`, `.matches()`, `.split()`, `.substring()`, `.length()`, `.push()`, `.join()`, `.at()`, `.get()`, `.has()`. Note: `.set(key, value)` is **Map-only** — there is no `array.set(i, v)`. To replace a single array element use `splice(i, 1, [v])`.
