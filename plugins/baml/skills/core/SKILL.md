...
- **Arrays have a JS-like method set** — `map`/`filter`/`filter_map`/`reduce`/`find`/`some`/`every`/`flat_map`/`slice`/`concat`/`join`/`includes`/length(), plus in-place `push`/`pop`/`shift`/`unshift`/`sort_by`/`sort_by_key`.
  - Mutating methods: `push(x) -> void`, `unshift(x) -> void`.
  - Optional-returning reads/removals: `pop() -> T?` (removed last element, or `null` if empty), `shift() -> T?` (removed first element, or `null` if empty), `at(i: int) -> T?` (element at `i`, or `null` if out of bounds).
  - Idiomatic unwrap:
    ```baml
    let last = xs.pop() ?? default_value
    let first = xs.shift() ?? default_value
    let item = xs.at(2) ?? default_value
    ```
  Most take closures that can `throws`. `baml describe Array` gives more info.
