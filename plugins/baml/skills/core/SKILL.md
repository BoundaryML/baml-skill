...
- **Test runners with `with`.** Put `with <runner>` after a `test` or `testset` name. A test accepts any `testing.TestRunner`; built-ins include `testing.Quorum(n, m)` and `testing.Retry(max_attempts)`. A testset accepts any `testing.TestSetRunner`; built-ins include `testing.PassRate(threshold)`, `testing.Sequential()`, and `testing.FailFast()`.

```baml
test "stable extraction" with testing.Quorum(5, 3) {
    assert.is_true(true)
}

testset "suite" with testing.FailFast() {
    test "first" { assert.is_true(true) }
    test "second" { assert.is_true(true) }
}
```
