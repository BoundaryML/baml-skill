...
- **Test runners:** `test "name" with runner { ... }` accepts any `testing.TestRunner` (built-ins: `testing.Quorum`, `testing.Retry`); `testset "name" with runner { ... }` accepts any `testing.TestSetRunner` (built-ins: `testing.PassRate`, `testing.Sequential`, `testing.FailFast`). Run `baml describe with` for details.
