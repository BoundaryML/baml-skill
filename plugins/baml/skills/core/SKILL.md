...
- **Value conversion.** `v.to_string()` produces human-readable text; `v.to_json()` produces a `baml.json.json` value and may throw `baml.json.JsonSerializationError`. Use `baml.json.stringify(v.to_json())` for JSON text. Both use structural defaults and honor `baml.ToString` / `baml.ToJson` overrides.
