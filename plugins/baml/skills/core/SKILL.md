...
- **Out-of-body + blanket implementations.** `implements I for T { … }` implements a method-only interface outside the type; add type parameters for a blanket rule: `implements<T extends Bound> I for T { … }`. Field-bearing interfaces must be implemented in the class body. `baml describe implements`.
...
implements Animal for Dog { function sound(self) -> string { "woof" } }
...
implements<T extends Animal> Loud for T {
    function shout(self) -> string { self.sound().to_upper_case() }
}
...
test "interfaces" {
    ...
    assert.equal(Dog { name: "Rex" }.shout(), "WOOF")
}
```
