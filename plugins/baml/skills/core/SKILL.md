...
- **Pattern matching.** `match (v) { … }` over values/types; arms are `pattern => expr` — literals, `let x: T` (including primitives such as `int`, `string`, and `float`), class destructure `T { f: let y }`, or-patterns `A | B`, guards `… if cond`, `_`; must be exhaustive. Also `v is T` → bool (narrows) and `if let x: T = v { … } else { … }`. `baml describe patterns`.
...
function type_name(x: int | string | float) -> string {
    match (x) { let n: int => "int", let s: string => "string", let f: float => "float" }
}
...
test "patterns" {
    ...
    assert.equal([type_name(1), type_name("x"), type_name(1.5)], ["int", "string", "float"]);
